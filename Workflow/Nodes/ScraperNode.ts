...
const metadata: NodeMetadata = {
  ...
  configParameters: [
    ...
    },
    {
      ...
      valueSource: "UserInput",
      UIConfigurable: true,
      description: "Parallel scraping threads (max 100 for ScraperAPI Business plan)",
      isNodeBodyContent: false,
    },
    {
    ...
    process: async (context: NodeExecutionContext) => {
      try {
        ...
        if (!searchResultsInput || !queryMetadataInput) {
          throw new Error("Both Search Results and Query Metadata inputs are required");
        }

        console.log(`[Scraper Node ${id}] Starting scraping and topic extraction...`);
        ...
        const scraperApiKey = getStringConfig(
          context.node.configParameters?.find(p => p.parameterName === "ScraperAPI Key")
        );
        const geminiApiKey = getStringConfig(
          context.node.configParameters?.find(p => p.parameterName === "Gemini API Key")
        );

        if (!scraperApiKey) {
          throw new Error("ScraperAPI Key is required. Please configure it in node settings.");
        }
        if (!geminiApiKey) {
          throw new Error("Gemini API Key is required. Please configure it in node settings.");
        }
        const geminiModel = getStringConfig(
          context.node.configParameters?.find(p => p.parameterName === "Gemini Model")
        ) || "gemini-2.5-flash";

        const concurrentScrapes = getNumberConfig(
          context.node.configParameters?.find(p => p.parameterName === "Concurrent Scrapes")
        ) || 5;

        const topicConcurrency = getNumberConfig(
          context.node.configParameters?.find(p => p.parameterName === "Topic Extraction Concurrency")
        ) || 4;

        const maxChunkSize = getNumberConfig(
          context.node.configParameters?.find(p => p.parameterName === "Max Chunk Size")
        ) || 7000;

        console.log(`[Scraper Node ${id}] Config: Model=${geminiModel}, ConcurrentScrapes=${concurrentScrapes}, TopicConcurrency=${topicConcurrency}`);

        console.log(`[Scraper Node ${id}] Step 1/4: Filtering ${searchResults.length} URLs...`);
        
        const filteredUrls = await filterUrls(searchResults);
        
        console.log(`[Scraper Node ${id}] Filtered to ${filteredUrls.length} valid URLs`);

        if (filteredUrls.length === 0) {
          throw new Error("No valid URLs remaining after filtration");
        }

        console.log(`[Scraper Node ${id}] Step 2/4: Scraping ${filteredUrls.length} URLs...`);
        
        const scrapedData = await scrapeUrls(filteredUrls, scraperApiKey, concurrentScrapes);
        
        const successfulScrapes = scrapedData.filter(d => d.success).length;
        console.log(`[Scraper Node ${id}] Successfully scraped ${successfulScrapes}/${scrapedData.length} pages`);
        ...
        console.log(`[Scraper Node ${id}] Created ${chunks.length} content chunks`);

        console.log(`[Scraper Node ${id}] Step 4/4: Extracting topics from chunks...`);
        ...
        console.log(`[Scraper Node ${id}] Extracted ${extractedTopics.length} unique topics`);

        const topicsOutput = {
          extracted_topics: extractedTopics,
          topics_count: extractedTopics.length,
          required_topics: queryMetadata.required_topics,
          coverage_ratio: extractedTopics.length / queryMetadata.required_topics,
        };
        ...
        console.log(`[Scraper Node ${id}] Scraping and extraction completed successfully`);

        return {
          [id * 100 + 3]: JSON.stringify(topicsOutput, null, 2),        
          [id * 100 + 4]: JSON.stringify(scrapedDataOutput, null, 2),   
          [id * 100 + 5]: `Success: Scraped ${successfulScrapes} pages, extracted ${extractedTopics.length} topics (${(topicsOutput.coverage_ratio * 100).toFixed(1)}% coverage)`,
        };
        ...
        return {
          [id * 100 + 3]: JSON.stringify({ extracted_topics: [], topics_count: 0, required_topics: 0, coverage_ratio: 0 }, null, 2),
          [id * 100 + 4]: JSON.stringify({ scraped_pages: 0, successful_scrapes: 0, failed_scrapes: 0, total_chunks: 0, scraped_content: [] }, null, 2),
          [id * 100 + 5]: `Error: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
...
async function scrapeUrls(
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    console.log(`  [Scraper] Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} URLs)`);
    ...
    for (let i = 0; i < batchResults.length; i++) {
      ...
      if (result.status === 'fulfilled') {
        ...
        console.warn(`  [Scraper] Failed to scrape ${currentUrl}:`, result.reason);
        results.push({
          url: currentUrl,
          title: "",
          content: "",
...
function chunkContent(scrapedData: ScrapedContent[], maxChunkSize: number): ContentChunk[] {
...
async function extractTopics(
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    if (allTopics.length >= requiredTopics) {
      console.log(`  [Topics] Reached required topic count (${requiredTopics}), stopping extraction`);
      break;
    }
    ...
    console.log(`  [Topics] Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} chunks)`);

    const batchPromises = batch.map(chunk => {
      if (!chunk || !chunk.content) {
        return Promise.resolve([]);
      }
      return extractTopicsFromChunk(chunk.content, language, domainType, apiKey, model);
    });
