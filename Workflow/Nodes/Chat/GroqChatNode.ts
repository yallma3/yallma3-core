...
const metadata: NodeMetadata = {
  ...
  nodeType: "GroqChat",
  description:
    "Integrates with the Groq API for high-speed chat completions. Connect User Input to Prompt Loop for a multi-turn chat loop, or to Prompt for a single call.",
  ...
  configParameters: [
    {
      ...
      sourceList: [
        { key: "llama-3.1-8b-instant",       label: "Llama 3.1 8b Instant"  },
        { key: "llama-3.3-70b-versatile",    label: "Llama 3.3 70b"         },
        { key: "llama3-70b-8192",            label: "Llama3 70b"            },
        { key: "mixtral-8x7b-32768",         label: "Mixtral 8x7b"          },
        { key: "gemma2-9b-it",               label: "Gemma 2 9b"            },
      ],
      i18n: {
    ...
    },
    {
      ...
      i18n: {
        en: { "API Key": { Name: "API Key", Description: "API Key for the Groq service" } },
        ar: { "API Key": { Name: "مفتاح API", Description: "مفتاح API لخدمة Groq" } },
      },
    },
  ],
  i18n: {
    en: {
      ...
      nodeType: "Groq Chat",
      description:
        "Integrates with the Groq API for high-speed chat completions. Supports single-turn and multi-turn loop mode.",
    },
    ar: {
      ...
      description:
        "يتكامل مع Groq API لإتمام المحادثات عالية السرعة. يدعم الوضع الأحادي ووضع الحلقة متعددة الأدوار.",
    },
  },
};
...
export function createNGroqChatNode(id: number, position: Position): ChatNode {
  return {
    ...
    process: async (context: NodeExecutionContext) => {
      ...
      if (!GROQ_API_KEY) {
        return {
          [n.id * 100 + 4]: "Error: Groq API Key not found",
          [n.id * 100 + 5]: 0,
        };
      }

      try {
        console.log(`Executing Groq Chat node ${n.id} with model: ${model}`);
        console.log(
        ...
        const output = json.choices?.[0]?.message?.content || "No response from Groq";

        console.log(
          `Groq node ${n.id} received response: ${output.substring(0, 50)}...`
        );
        ...
        console.error("Error in Groq Chat node:", error);
        return {
          [n.id * 100 + 4]: `Error: ${
