    ...
    process: async (context: NodeExecutionContext) => {
      try {
        ...
        return {
          ...
          [id * 100 + 5]: `Success: Published ${datasetData.metadata.total_rows} rows to HuggingFace at ${repoUrl}`,
        };
      } catch (error) {
...
- text-classification
size_categories:
- ${metadata.total_rows < 1000 ? "n<1K" : metadata.total_rows < 10000 ? "1K<n<10K" : "10K<n<100K"}
tags:
- synthetic
...
This is a synthetic dataset generated using the yaLLMa3 pipeline for ${metadata.data_type} tasks in ${metadata.language}.
...
export function register(nodeRegistry: NodeRegistry): void {
  console.log(
    `Registering ${metadata.title} Node under category: ${metadata.category}`
  );
