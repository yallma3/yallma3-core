...
topics.forEach((topic, index) => {
  topicBatches[index % parallelAgents]?.push(topic);
        ...
        console.log(
          ...
          topicBatches.map((batch, i) => `Agent ${i + 1}: ${batch.length} topics`)
        );
        console.log(
        ...
        return {
          ...
          [id * 100 + 5]: `Success: Generated ${allData.length} data points from ${successfulTopics}/${topics.length} topics (${statistics.success_rate}% success rate) in ${executionTime}s`,
        };
      } catch (error) {
...
async function generateDataForTopics(
  ...
  for (let i = 0; i < topics.length; i++) {
    ...
    if (!topic || typeof topic !== "string") {
      console.warn(`  [Agent ${agentIndex}] Skipping invalid topic at index ${i}`);
      failed++;
      continue;
    }
