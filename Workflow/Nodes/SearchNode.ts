  ...
  return {
    ...
    process: async (context: NodeExecutionContext) => {
      try {
        ...
        if (!tavilyApiKey) {
          throw new Error(
            "Tavily API Key is required. Please configure it in node settings."
          );
        }
        ...
        const numQueries =
          (queryCountParam?.paramValue as number) ||
          (queryCountParam?.defaultValue as number);
        ...
        const resultsPerQuery =
          (resultsPerQueryParam?.paramValue as number) ||
          (resultsPerQueryParam?.defaultValue as number);

        console.log(
          `[Search Node ${id}] Config: Model=${geminiModel}, Queries=${numQueries}, ResultsPerQuery=${resultsPerQuery}`
        );

        console.log(
          `[Search Node ${id}] Step 1/3: Parsing user query with Gemini...`
        );

        const parsedQuery = await parseUserQuery(
          userQuery,
          geminiApiKey,
          geminiModel
        );
        ...
        if (parsedQuery.query_type === "not_data_generation") {
          throw new Error(
            "This query is not related to data generation. Please provide a data generation request."
          );
        }

        if (parsedQuery.query_type === "incomplete") {
          throw new Error(
            "Your request is incomplete. Please specify: number of rows, language, data description, and data type."
          );
        }

        console.log(
          `[Search Node ${id}] Parsed query:`,
        ...
        console.log(
          `[Search Node ${id}] Step 2/3: Generating ${numQueries} refined search queries...`
        );
        ...
        console.log(
          `[Search Node ${id}] Generated ${refinedQueries.length} refined queries`
        );
        console.log(
          `[Search Node ${id}] Step 3/3: Executing web search via Tavily...`
        );
        ...
        console.log(
          `[Search Node ${id}] Collected ${searchResults.length} total URLs`
        );
        ...
        return {
          ...
          [id * 100 +
          4]: `Success: Generated ${refinedQueries.length} queries, found ${searchResults.length} URLs for ${parsedQuery.data_type} data in ${parsedQuery.language}`,
        };
      } catch (error) {
        console.error(`[Search Node ${id}]  Error:`, error);

        return {
          [id * 100 + 2]: JSON.stringify(
            { search_results: [], total_count: 0 },
            null,
            2
          ),
...
async function executeWebSearch(
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    try {
      console.log(
        `  [Tavily] Searching (${i + 1}/${queries.length}): "${query}"`
      );
      ...
      if (!response.ok) {
        ...
        console.warn(
          `  [Tavily] Search failed for query "${query}": ${errorText}`
        );
        continue;
      }
