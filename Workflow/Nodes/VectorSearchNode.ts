...
export function register(nodeRegistry: NodeRegistry): void {
  ...
  function createPineconeSearchNode(id: number, position: Position): PineconeSearchNode {
    return {
      id,
        ...
        try {
          ...
          if (Array.isArray(rawVector) && rawVector.every(v => typeof v === "number")) {
            ...
            try {
              ...
              throw new Error("Invalid vector input. Must be number[] or JSON stringified number[].");
            }
          }
          ...
          if (!apiKey) throw new Error("Pinecone API Key is required (set in config).");
          ...
          return {
            [n.id * 100 + 2]: result,
            [n.id * 100 + 3]: `Query returned ${result.matches?.length ?? 0} results`,
          };
        } catch (error) {
          return {
            [n.id * 100 + 2]: null,
  ...
  nodeRegistry.registerNodeType("VectorSearch", createPineconeSearchNode, metadata);
}